...
const Button: Component<Props> & CompoundComponents = ({ children, template, border = false, onClick }) => {
    return (
        <button onClick={onClick} className={cn(styles.button, typeButon(template), border && styles.border)}>
            {children}
        </button>)
}

const Text: Component<{size?: Size}> = ({ children, size = Size.SMALL }) => (<span classList={{
